...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, callback) {
	debug('generateAuthorizeRequest called');
...
SAML.prototype.generateServiceProviderMetadata = function( decryptionCert ) {
  ...
  var metadata = {
	...
	var xml =  xmlbuilder.create(metadata).end({ pretty: true, indent: '  ', newline: '\n' });
	debug('Service provider metadata %s', xml);
  return xml;
};
